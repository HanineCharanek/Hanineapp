...
if section == "Overview":
    # Set page title
    st.markdown("<h1 style='color: darkblue; font-size: 32px;'>Overview about Covid-19 </h1>", unsafe_allow_html=True)

    import streamlit as st

    def main():
        # First Column
        st.markdown("## What is the coronavirus that causes COVID-19?😷")
        ...
        # Second Column
        st.markdown("## How does the coronavirus causing COVID-19 spread?🤧")
        st.markdown("**Direct:**")
        ...
        # Third Column
        st.markdown("## In case you have to travel to an affected country, please take the following precautions 🤒")
...
if section == "Home":
    # Set page title
    st.markdown("<h1 style='color: darkblue; font-size: 32px;'>COVID-19 and Vaccine Tracker in Europe Dashboard</h1>", unsafe_allow_html=True)
...
if section == "Data Exploration and Analysis":
    ...
    if page == "Data on 14-day notification rate of new COVID-19 cases and deaths":
        st.title("14-day notification rate of new COVID-19 cases and deaths")
       ...
       # Page 2: Data on COVID-19 vaccination in the EU/EEA
        ...
        st.title(" COVID-19 vaccination in the EU/EEA")
         
           # Read the data from the CSV file
        data_path = r'C:\Users\student\Desktop\data (2).csv'
        ...
        def visualize_data():
            # Read the data from the CSV file
            data_pathh = r"C:\Users\student\Desktop\data (2).csv"
            vaccine_df = pd.read_csv(data_pathh)
     ...
     # Page 3: Data on hospital and ICU admission rates and current occupancy for COVID-19
        ...
        st.title(" Hospital and ICU admission rates and current occupancy for COVID-19")
        ...
        def main():
            ...
            # Group the data by country and calculate the average value for each country
            grouped_data1 = filtered_data1.groupby('country')['value'].mean().reset_index()
            ...
            ax1.bar(grouped_data1['country'], grouped_data1['value'])
            ax1.set_title(f'{indicator1} across different countries')
            ax1.set_xlabel('Country')
            ax1.set_ylabel(f'{indicator1}')
            ax1.set_xticklabels(grouped_data1['country'], rotation=45)
            ax1.grid(True)
            ...
            # Pivot the data to create a matrix with dates as index, countries as columns, and occupancy values as values
            pivoted_data = filtered_data2.pivot(index='date', columns='country', values='value')

            # Calculate the cumulative sum of occupancy for each country
            cumulative_data = pivoted_data.cumsum(axis=0)

            # Create the stacked area chart for indicator2
            fig2, ax2 = plt.subplots(figsize=(12, 8))
            ax2.stackplot(cumulative_data.index, cumulative_data.values.T, labels=cumulative_data.columns)
            ax2.set_title(f'Cumulative {indicator2} by Country over Time')
            ax2.set_xlabel('Date')
            ax2.set_ylabel('Cumulative Occupancy')
            ax2.legend(loc='upper left')
            ax2.set_xticklabels(cumulative_data.index, rotation=45)

            # Display the visuals side by side using Streamlit
            col1, col2 = st.columns(2)
            with col1:
                # Allow the user to select a country for the bar chart
                selected_country1 = st.selectbox('Select a country', grouped_data1['country'])
                filtered_data1_country = filtered_data1[filtered_data1['country'] == selected_country1]
                st.markdown("<h2 style='font-size: 24px;'>Weekly new hospital admissions per 100k'</h2>", unsafe_allow_html=True)
                fig1_country, ax1_country = plt.subplots(figsize=(10, 6))
                ax1_country.bar(filtered_data1_country['date'], filtered_data1_country['value'])
                ax1_country.set_title(f'{indicator1} in {selected_country1}')
                ax1_country.set_xlabel('Date')
                ax1_country.set_ylabel(f'{indicator1}')
                ax1_country.set_xticklabels(filtered_data1_country['date'], rotation=45)
                ax1_country.grid(True)
                st.pyplot(fig1_country)
